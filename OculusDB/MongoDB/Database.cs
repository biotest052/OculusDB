...
public class OculusDBDatabase
{
    ...
    public static void Initialize()
    {
        ...
        CreateAscendingIndex(achievementCollection, x => x.grouping.id);
        
        CreateAscendingIndex(appImages, x => x.parentApplication.id);
        
        CreateAscendingIndex(applicationCollection, x => x.id);
        CreateTextIndex(applicationCollection, x => x.searchDisplayName);
        CreateTextIndex(applicationCollection, x => x.canonicalName);
        CreateTextIndex(applicationCollection, x => x.packageName);
        CreateTextIndex(applicationCollection, x => x.publisherName);
        
        CreateAscendingIndex(differenceCollection, x=> x.differenceName);
        CreateAscendingIndex(differenceCollection, x=> x.entryParentApplicationIds);
        
        CreateAscendingIndex(iapItemCollection, x => x.id);
        CreateAscendingIndex(iapItemCollection, x => x.grouping.id);
        CreateTextIndex(iapItemCollection, x => x.displayName);
        CreateTextIndex(iapItemCollection, x => x.sku);
        
        CreateAscendingIndex(iapItemPackCollection, x => x.id);
        CreateAscendingIndex(iapItemPackCollection, x => x.grouping.id);
        CreateTextIndex(iapItemPackCollection, x => x.displayName);
        
        CreateAscendingIndex(offerCollection, x => x.id);
        CreateAscendingIndex(offerCollection, x => x.currency);
        CreateAscendingIndex(offerCollection, x => x.grouping.id);
        
        CreateAscendingIndex(versionCollection, x => x.id);
        CreateAscendingIndex(versionCollection, x => x.parentApplication.id);
        CreateAscendingIndex(versionCollection, x => x.filename);
    ...
    public static void CreateAscendingIndex<T>(IMongoCollection<T> collection,Expression<Func<T, object>> field)
    {
        collection.Indexes.CreateOneAsync(new CreateIndexModel<T>(Builders<T>.IndexKeys.Ascending(field)));
    }
    public static void CreateTextIndex<T>(IMongoCollection<T> collection,Expression<Func<T, object>> field)
    {
        collection.Indexes.CreateOneAsync(new CreateIndexModel<T>(Builders<T>.IndexKeys.Text(field)));
    }
